//! The scene command executor: one JSON command in, one JSON reply out,
//! executed against the live scene on the render-loop thread.
//!
//! Two callers share it: the MCP control server (`control.rs`, native only)
            ...
            let (default_name, bytes): (String, Vec<u8>) =
                if let Some(path) = command["path"].as_str() {
                    #[cfg(target_arch = "wasm32")]
                    {
                        let _ = path;
                        return Err(
                            "'path' is unavailable in the browser — pass 'data_base64'".to_string()
                        );
                    }
...
/// returns rows top-down, so no flip is needed. (Native only in practice:
